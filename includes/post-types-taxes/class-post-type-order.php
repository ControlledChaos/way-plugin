...
if ( way_acf_options() ) {
    $sort_order = get_field( 'way_use_custom_sort_order', 'option' );

// If Advanced Custom Fields is not active.
} else {
    $sort_order = get_option( 'way_use_custom_sort_order' );
}
...
class Post_Types_Taxes_Order {
	...
	public static function instance() {
		...
		if ( is_null( $instance ) ) {
            ...
            if ( ! get_option( 'way_order_install ' ) ) {
    ...
    public function order_install() {
        ...
        update_option( 'way_order_install', 1 );
    ...
    public function admin_menu() {
        ...
        add_options_page(
            __( 'Posts & Taxonomies Sort Order', 'way-plugin' ),
            __( 'Sort Order', 'way-plugin' ),
            'manage_options',
            'sort-order-settings',
            [ $this, 'admin_page' ]
        );
    ...
    public function admin_page() {
        ...
        require WAY_PATH . 'admin/partials/settings-page-posts-order.php';
    ...
    public function load_script_css() {
        ...
        if ( $this->_check_load_script_css() ) {
            ...
            wp_enqueue_script( 'post-tax-order', WAY_URL . 'admin/assets/js/post-tax-order.js', [ 'jquery' ], null, true );
    ...
    public function update_options() {
        ...
        if ( ! isset( $_POST['way_posts_order_submit'] ) ) {
            return false;
        }

        check_admin_referer( 'way_posts_order_nonce' );

        $input_options            = [];
        $input_options['objects'] = isset( $_POST['objects'] ) ? $_POST['objects'] : '';
        $input_options['tags']    = isset( $_POST['tags'] ) ? $_POST['tags'] : '';

        update_option( 'way_order_options', $input_options );
    ...
    public function get_order_options_objects() {
        ...
        if ( $way_order_options = get_option( 'way_order_options' ) ) {
            $way_order_options = get_option( 'way_order_options' );
        } else {
            $way_order_options = [];
        }

        if ( isset( $way_order_options['objects'] ) && is_array( $way_order_options['objects'] ) ) {
            $objects = $way_order_options['objects'];
        } else {
            $objects = [];
        }
    ...
    public function get_order_options_tags() {
        ...
        if ( isset( $way_order_options['tags'] ) && is_array( $way_order_options['tags'] ) ) {
            $tags = $way_order_options['tags'];
        } else {
            $tags = [];
        }
 ...
 */
function way_post_types_taxes_order() {
...
function uninstall_db() {
    ...
    delete_option( 'way_order_install' );
