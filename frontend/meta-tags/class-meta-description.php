...
class Meta_Description {
	...
	public function __construct() {
		...
		add_action( 'way_meta_description_tag', [ $this, 'description' ] );
	...
	public function description() {
		...
		if ( way_acf_options() ) {
			 ...
			 */
			$acf_blog_desc = get_field( 'way_meta_blog_description', 'option' );
			...
			$wp_blog_desc = get_option( 'way_meta_blog_description' );
		...
		$search_desc      = wp_strip_all_tags(
			sprintf(
				'%1s \'%2s\'',
				__( 'Showing results for', 'way-plugin' ),
				$search_query
			)
		);

		// Apply a filter to hard-coded text.
		$search_meta_desc = apply_filters( 'way_meta_description_search', $search_desc );
		...
		$meta_description = apply_filters( 'way_meta_description', $description );
