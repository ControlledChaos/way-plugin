...
class Admin {
	...
	private function dependencies() {
		...
		require_once WAY_PATH . 'admin/class-settings.php';

		// Add icons to the titles of ACF tab and accordion fields, if active.
		if ( way_acf_pro() && ! get_option( 'way_acf_activate_settings_page' ) ) {
			include_once WAY_PATH . 'admin/class-acf-tab-icons.php';
		}

		// Include custom fields for Advanced Custom Fields Pro, if active.
		if ( way_acf_pro() ) {
			include_once WAY_PATH . 'admin/class-settings-fields-site-acf.php';
		}

		// Restore the TinyMCE editor.
		if ( way_acf_pro() ) {
			$editor = get_field( 'way_classic_editor', 'option' );
		} else {
			$editor = get_option( 'way_classic_editor' );
		}
		if ( ( way_classicpress() || way_new_cms() ) && $editor ) {
			include_once WAY_PATH . 'admin/classic-editor/classic-editor.php';
		}

		// Functions for dasboard widgets, excluding the welcome panel.
		require_once WAY_PATH . 'admin/dashboard/class-dashboard.php';

		// Functions for admin menu item positions and visibility.
		require_once WAY_PATH . 'admin/class-admin-menu.php';

		// Add menus to the admin toolbar.
		require_once WAY_PATH . 'admin/class-admin-toolbar-menus.php';

		// Functions for various admin pages and edit screens.
		require_once WAY_PATH . 'admin/class-admin-pages.php';

		// Import custom fields for editing, if ACF Pro is active.
		if ( way_acf_options() ) {
			include_once WAY_PATH . 'admin/class-fields-import.php';
		}

		// Filter by page template.
		require_once WAY_PATH . 'admin/class-admin-template-filter.php';
	...
	public function admin_footer() {
		...
		if ( way_classicpress() ) {
			$default = __( 'website powered by ClassicPress', 'way-plugin' );

		// If the site is running WordPress.
		} else {
			$default = __( 'website powered by WordPress', 'way-plugin' );
		}
		...
		if ( way_acf_options() ) {
			 ...
			 */
			$credit = get_field( 'way_admin_footer_credit', 'option' );
			$link   = get_field( 'way_admin_footer_link', 'option' );
			...
			if ( $credit && $link ) {
				$footer = sprintf(
					...
					$site,
					esc_html__( 'website designed & developed by', 'way-plugin' ),
					esc_url( $link ),
					$credit
				);
		 ...
		 */
			...
			$credit = sanitize_text_field( get_option( 'way_footer_credit' ) );
			$link   = esc_url_raw( get_option( 'way_footer_link' ) );
		...
		$admin_footer = apply_filters( 'way_admin_footer', $footer );
	...
	public function enqueue_styles() {
		 ...
		 * @since 1.0.0
		 */
		wp_enqueue_style( WAY_ADMIN_SLUG . '-admin', WAY_URL . 'admin/assets/css/admin.min.css', [], WAY_VERSION, 'all' );
		 ...
		 * @since 1.0.0
		 */
		wp_enqueue_style( WAY_ADMIN_SLUG . '-tooltips', WAY_URL . 'admin/assets/css/tooltips.min.css', [], WAY_VERSION, 'all' );
		 ...
		 */
		if ( way_acf() ) {
			wp_enqueue_style( WAY_ADMIN_SLUG . '-acf', WAY_URL . 'admin/assets/css/acf.css', [], WAY_VERSION, 'all' );
		}
		 ...
		 * @since 1.0.0
		 */
		$welcome = get_option( 'way_custom_welcome' );
		if ( $welcome ) {
			wp_enqueue_style( WAY_ADMIN_SLUG . '-welcome', WAY_URL . 'admin/assets/css/welcome.css', [], WAY_VERSION, 'all' );
		}
	...
	public function get_search_form() {
		...
		require WAY_PATH . 'admin/partials/searchform.php';
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_script( WAY_ADMIN_SLUG . '-conditionalize', WAY_URL . 'admin/assets/js/admin.js', [ 'jquery' ], WAY_VERSION, true );

		// Enqueue scripts for backend functionality of this plugin.
		wp_enqueue_script( WAY_ADMIN_SLUG . '-admin', WAY_URL . 'admin/assets/js/conditionize.flexible.jquery.min.js', [ 'jquery' ], WAY_VERSION, true );
