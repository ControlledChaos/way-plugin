...
class Meta_SEO_Callbacks {
	...
	public function disable_meta( $args ) {
		...
		$option = get_option( 'way_meta_disable' );

		$html = '<p><input type="checkbox" id="way_meta_disable" name="way_meta_disable" value="1" ' . checked( 1, $option, false ) . '/>';

		$html .= '<label for="way_meta_disable"> '  . $args[0] . '</label></p>';
	...
	public function  schema_org_type( $args ) {
		...
		// Get the field file.
		include_once WAY_PATH . 'admin/partials/field-callbacks/schema-org-type.php';
 ...
 */
function way_meta_seo_callbacks() {
