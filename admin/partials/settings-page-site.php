...
$tabs = [
    ...
    sprintf(
        ...
        esc_html__( 'Dashboard', 'way-plugin' )
    ),
    ...
    sprintf(
        ...
        esc_html__( 'Admin Menu', 'way-plugin' )
    ),
    ...
    sprintf(
        ...
        esc_html__( 'Admin Pages', 'way-plugin' )
    ),
    ...
    sprintf(
        ...
        esc_html__( 'Site Users', 'way-plugin' )
    ),
    ...
    sprintf(
        ...
        esc_html__( 'Meta/SEO', 'way-plugin' )
    )

];

// Apply a filter to the tabs array for adding tabs.
$page_tabs = apply_filters( 'way_tabs_site_settings', $tabs );
...
if ( 'dashboard' == $active_tab  ) {
    $section = 'way-site-dashboard';
    $fields  = 'way-site-dashboard';
} elseif ( 'admin-menu' == $active_tab ) {
    $section = 'way-site-admin-menu';
    $fields  = 'way-site-admin-menu';
} elseif ( 'admin-pages' == $active_tab ) {
    $section = 'way-site-admin-pages';
    $fields  = 'way-site-admin-pages';
} elseif ( 'users' == $active_tab ) {
    $section = 'way-site-users';
    $fields  = 'way-site-users';
} elseif ( 'meta-seo' == $active_tab ) {
    $section = 'way-site-meta-seo';
    $fields  = 'way-site-meta-seo';
} else {
    $section = null;
    $fields  = null;
}
...
$do_section = apply_filters( 'way_section_site_settings', $section );
$do_fields  = apply_filters( 'way_fields_site_settings', $fields );
 ...
 */
if ( 'dashboard' == $active_tab  ) {
    $save = __( 'Save Dashboard', 'way-plugin' );
} elseif ( 'admin-menu' == $active_tab ) {
    $save = __( 'Save Menu', 'way-plugin' );
} elseif ( 'admin-pages' == $active_tab ) {
    $save = __( 'Save Pages', 'way-plugin' );
} elseif ( 'users' == $active_tab ) {
    $save = __( 'Save Users', 'way-plugin' );
} elseif ( 'meta-seo' == $active_tab ) {
    $save = __( 'Save Meta', 'way-plugin' );
} else {
    $save = __( 'Save Settings', 'way-plugin' );
}

// Apply a filter for new tabs added by another plugin or from a theme.
$button = apply_filters( 'way_save_site_settings', $save );
...
<div class="wrap">
	<?php echo sprintf(
        ...
        get_bloginfo( 'name' ),
        esc_html__( 'Settings', 'way-plugin' )
    ); ?>
    <p class="description"><?php esc_html_e( 'Customize the way WordPress is used.', 'way-plugin' ); ?></p>
