...
class Welcome {
    ...
    public function __construct() {
		...
		if ( way_acf_options() ) {
			$dismiss = get_field( 'way_remove_welcome_dismiss', 'option' );

		// If ACF is not active, get the field from the WordPress/ClassicPress options page.
		} else {
			$dismiss = get_option( 'way_remove_welcome_dismiss' );
		}
		...
		// If ACF is active, get the field from the ACF options page.
		if ( way_acf_options() ) {
			$welcome = get_field( 'way_custom_welcome', 'option' );
		} else {
	...
	public function widget_areas() {
		...
		register_sidebar( [
			'name'          => __( 'Welcome Panel - First Area', 'way-plugin' ),
			'id'            => 'way_welcome_widget_first',
			'description'   => __( '', 'way-plugin' ),
		...
		register_sidebar( [
			'name'          => __( 'Welcome Panel - Second Area', 'way-plugin' ),
			'id'            => 'way_welcome_widget_second',
			'description'   => __( '', 'way-plugin' ),
			'before_widget' => '<div id="%1$s" class="widget %2$s">',
			'after_widget'  => '</div>',
			'before_title'  => '<h3>',
		...
		register_sidebar( [
			'name'          => __( 'Welcome Panel - Third Area', 'way-plugin' ),
			'id'            => 'way_welcome_widget_last',
			'description'   => __( '', 'way-plugin' ),
			'before_widget' => '<div id="%1$s" class="widget %2$s">',
			'after_widget'  => '</div>',
			'before_title'  => '<h3>',
	...
	public function welcome_panel() {
		...
		if ( ! empty( $welcome ) ) {
			...
			include_once WAY_PATH . 'admin/dashboard/partials/welcome-panel.php';
		}
	...
	public function styles() {
        ...
        if ( $screen->id == 'dashboard' ) {
            wp_enqueue_style( WAY_ADMIN_SLUG . '-welcome', WAY_URL .  'assets/css/welcome.min.css', [], null, 'screen' );
        }
