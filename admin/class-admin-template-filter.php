...
class Filter_By_Template {
	...
	public function filter_dropdown() {
		...
		<select name="page_template_filter" id="page_template_filter">
			<option value="all"><?php _e( 'All Page Templates', 'way-plugin' ); ?></option>
			<option value="default" <?php echo ( $template == 'default' ) ? ' selected="selected" ' : ''; ?>><?php echo _e( 'Default Template', 'way-plugin' ); ?></option>
	...
	public function template_columns_head( $columns ) {
		...
		$columns['template'] = __( 'Template', 'way-plugin' );
	        ...
	        if ( $template ) {
	        	...
	        	if ( $template == 'default' ) {
					...
					echo sprintf(
						...
						__( 'Default Template', 'way-plugin' ),
						__( 'Default Template', 'way-plugin' )
					);
				...
				// If it's not the default template.
					...
					// If the template cannot be found.
	        			...
	        			echo sprintf(
							'<span title="%1s">%2s</span>',
							__( 'This template file does not exist', 'way-plugin' ),
							$template
						);
					}
 ...
 */
function way_filter_by_template() {
